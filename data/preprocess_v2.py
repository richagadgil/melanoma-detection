...
def gen_n_split_data(dataset, trainset, testset, validset):
    
    set_len = len(dataset)

    if set_len < sets_per_class:
        remainder = sets_per_class - set_len
...
def main():
    ...
    with open("disease_dataset.csv", mode="w") as outfile:
        ...
        for data in trainset.keys():
            ...
            label = determine_class(data)
            shutil.copyfile(os.getcwd() + "\\ISIC_2019_Training_Input\\ISIC_2019_Training_Input\\" + data + ".jpg", os.getcwd() + "\\tf_dataset\\"  + label + "\\" + data + "_"  + label + ".jpg")
